...
out vec4 fragColor;
void main() {
	fragColor = sqrt(texture(framebuff, fragCoord));
}
`;
...
const Sphere objs[9] = Sphere[9](
	...
	Sphere(vec3(0, 0.1, 2), 0.6, 0.0, vec3(1,1,1), Material(0.0, 0.0, 1.0, 1.2)),
	...
	Sphere(vec3(-3, -3, -5), 0.3, 100.0, vec3(0.7, 0.2, 0.8), Material(1.0, 0.0, 0.0, 0.0))
	//Sphere(vec3(0, -100.3, 0), 100.0, 0.0, vec3(0.5, 0.7, 0.9), Material(1.0, 0.0, 0.0, 0.0))
);
...
void main() {
	...
	clr /= samples;
	fragColor = vec4(mix(clr, previous, (acc_samples / (acc_samples + samples))), 1.0);
}
`;